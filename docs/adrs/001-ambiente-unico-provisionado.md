...
A spec `001` mapeou `develop` → homologação e `main` → produção, com deploy real nos quatro repositórios. Isso implica dois ambientes AWS completos (dois EKS, dois RDS, duplicação de Lambda, Gateway e mensageria).
