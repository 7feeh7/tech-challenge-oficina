...
function validarCnpj(cnpj: string): boolean {
  ...
  const calcDigit = (d: string, weights: number[]) => {
    const sum = weights.reduce((acc, w, i) => acc + Number.parseInt(d[i]) * w, 0);
  ...
  return (
    calcDigit(digits, w1) === Number.parseInt(digits[12]) &&
    calcDigit(digits, w2) === Number.parseInt(digits[13])
  );
}
